...
export type PackageType = { id: number; size_code: string; max_weight_kg: number; description: string; is_active: boolean };
export type Address = { id: number; street: string; exterior_number: string; interior_number?: string; neighborhood: string; postal_code: string; city: string; state?: string; country?: string; is_active: boolean };
...
const OrderModal: React.FC<OrderModalProps> = ({ open, mode, orderId, onClose, onSaved }) => {
  ...
  const computePackageTypeId = React.useCallback((weight: number) => {
    ...
    // sort by max_weight_kg ascending
    const sorted = [...pkgTypes].sort((a,b)=>a.max_weight_kg - b.max_weight_kg);
    const hit = sorted.find(p => weight <= p.max_weight_kg) ?? sorted[sorted.length - 1];
    return hit?.id ?? 0;
  }, [pkgTypes]);
  ...
  const fetchBaseData = React.useCallback(async () => {
    ...
    try {
      ...
      setAddresses(addrs);
      setPkgTypes(pkgs.filter(p=>p.is_active));
        ...
        <div className="row g-3">
          ...
          {isAdmin && (
            <div className="col-12">
              ...
              <textarea className="form-control" name="internal_notes" rows={3} value={form.internal_notes}
